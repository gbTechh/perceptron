...
int main() {
  cout << "=== PERCEPTRÓN CON EARLY STOPPING ===" << endl;
  ...
  for (int label = 0; label < CANT_PERCEPTRONS; label++) {
    perceptrones.push_back(
        Perceptron(label, train_data[0].pixels.size(), 0, 0.001, train_data));
  }

  cout << "\n=== ENTRENAMIENTO ===" << endl;
  for (auto &p : perceptrones) {
    // max_epocas=100, convergence_threshold=1e-5, patience=5
    p.entrenamiento(100, 1e-5, 5);
  }
